/**
 * Geometry Library - Color
 *
 * RGBA color class with HSV conversion and CSS string parsing.
 * Color values are in range 0-1 (not 0-255).
 */

import { expressionCodeForNumber } from './math.js';
...
export class Color {
    ...
    toCSSString() {
        ...
        if (this.a === 1) {
            // Use hex for maximum compatibility
    ...
    static fromCSSString(cssString) {
        ...
        if (rgba) {
            return new Color(
                ...
                fromAlphaString(rgba[4])
            );
        }

        // Not a valid color - return black
        return new Color();
    }
}
